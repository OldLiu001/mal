...
define(<<<ev_read_string2>>>, <<<ifelse(__ERR, 1, __EE, <<<define(<<<__RS>>>, unesc(inner_of(ev_form(defn(<<<__SF_ELEM>>>), defn(<<<__REPL_ENV>>>)))))patsubst(patsubst(patsubst(ENC(read_str(translit(defn(<<<__RS>>>), NLCH()TABCH()CRCH(), S1()S2()S3()))), S1(), <<<\\n>>>), S2(), <<<\\t>>>), S3(), <<<\\r>>>)>>>)>>>)dnl
dnl 转义解码：\n → 换行，\t → tab，\r → CR，\" → "，\\ → \，\其它 → 其它
...
define(<<<ev_slurp2>>>, <<<define(<<<__SLF>>>, inner_of(DEC(ev_form(defn(<<<__SF_ELEM>>>), defn(<<<__REPL_ENV>>>)))))define(<<<__SL>>>, esyscmd(<<<cat -- '>>>__SLF<<<' | tr '(),\043' '\016\017\020\021'>>>))define(<<<__SLB>>>, __SL)define(<<<__SLS>>>, <<<">>>translit(__SLB, NLCH()TABCH()CRCH(), S1()S2()S3())<<<">>>)patsubst(patsubst(patsubst(defn(<<<__SLS>>>), S1(), <<<\\n>>>), S2(), <<<\\t>>>), S3(), <<<\\r>>>)>>>)dnl
dnl ---- load-file ----
define(<<<ev_load_file>>>, <<<split_first(<<<$1>>>)ev_load_file2(defn(<<<__SF_ELEM>>>), defn(<<<__SF_REST>>>), <<<$2>>>)>>>)dnl
define(<<<ev_load_file2>>>, <<<define(<<<__LFF>>>, inner_of(DEC(ev_form(defn(<<<__SF_ELEM>>>), defn(<<<__REPL_ENV>>>)))))define(<<<__LF>>>, esyscmd(<<<cat -- '>>>__LFF<<<' | tr '(),\043' '\016\017\020\021'>>>))define(<<<__LFB>>>, substr(__LF, 0, eval(len(__LF)-1)))define(<<<__LFC>>>, patsubst(__LFB, <<<;.*>>>, <<<>>>))ev_lf_loop(translit(ENC(__LFC), NLCH(), SP()), <<<$3>>>)>>>)dnl
