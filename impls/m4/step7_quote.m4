...
define(<<<ev_qq>>>, <<<define(<<<__QQX>>>, qq_expand(<<<$1>>>))ev_form(defn(<<<__QQX>>>), <<<$2>>>)>>>)dnl
dnl quasiquote expansion (reference semantics: build cons/concat/quote/vec form, then EVAL)
dnl qq_expand: atom -> (quote x) / as-is; list -> foldr; vector -> (vec foldr); map -> (quote m)
define(<<<qq_expand>>>, <<<qq_xd(first_char(<<<$1>>>), <<<$1>>>)>>>)dnl
define(<<<qq_xd>>>, <<<ifelse(<<<$1>>>, LP, <<<qq_xlist(<<<$2>>>)>>>, <<<qq_xd2(<<<$1>>>, <<<$2>>>)>>>)>>>)dnl
define(<<<qq_xd2>>>, <<<ifelse(<<<$1>>>, <<<[>>>, <<<LP()<<<vec>>>SP()qq_xfold(inner_of(<<<$2>>>))RP()>>>, <<<qq_xd3(<<<$1>>>, <<<$2>>>)>>>)>>>)dnl
define(<<<qq_xd3>>>, <<<ifelse(<<<$1>>>, <<<{>>>, <<<LP()<<<quote>>>SP()<<<$2>>>RP()>>>, <<<qq_xatom(<<<$2>>>)>>>)>>>)dnl
define(<<<qq_xatom>>>, <<<ifelse(<<<$1>>>, nil, <<<$1>>>, <<<qq_xa2(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_xa2>>>, <<<ifelse(<<<$1>>>, true, <<<$1>>>, <<<qq_xa3(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_xa3>>>, <<<ifelse(<<<$1>>>, false, <<<$1>>>, <<<qq_xa4(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_xa4>>>, <<<ifelse(is_number(<<<$1>>>), 1, <<<$1>>>, <<<qq_xa5(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_xa5>>>, <<<ifelse(first_char(<<<$1>>>), <<<">>>, <<<$1>>>, <<<qq_xa6(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_xa6>>>, <<<ifelse(first_char(<<<$1>>>), <<<:>>>, <<<$1>>>, <<<LP()<<<quote>>>SP()<<<$1>>>RP()>>>)>>>)dnl
dnl list: () -> (); (unquote x) -> x; else foldr over elements
define(<<<qq_xlist>>>, <<<ifelse(len(inner_of(<<<$1>>>)), 0, <<<LP()RP()>>>, <<<split_first(inner_of(<<<$1>>>))ifelse(defn(<<<__SF_ELEM>>>), <<<unquote>>>, <<<skip_ws(defn(<<<__SF_REST>>>))>>>, <<<qq_xfold(inner_of(<<<$1>>>))>>>)>>>)>>>)dnl
dnl foldr (right-to-left): reverse elements, then build cons chain
dnl m4 arg evaluation is right-to-left; split_first results must be snapshotted
dnl into private globals before any recursive call that may clobber __SF_*
define(<<<qq_split>>>, <<<ifelse(first_char(<<<$1>>>), LP, <<<sf_paren(<<<$1>>>)>>>, <<<qq_sp2(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_sp2>>>, <<<ifelse(first_char(<<<$1>>>), <<<[>>>, <<<sf_bracket(<<<$1>>>)>>>, <<<qq_sp3(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_sp3>>>, <<<ifelse(first_char(<<<$1>>>), <<<{>>>, <<<sf_brace(<<<$1>>>)>>>, <<<split_first(<<<$1>>>)>>>)>>>)dnl
define(<<<qq_xfold>>>, <<<qq_xrev(<<<$1>>>, __EE)>>>)dnl
define(<<<qq_xrev>>>, <<<ifelse(len(<<<$1>>>), 0, <<<qq_xbuild(<<<$2>>>, LP()RP())>>>, <<<qq_split(<<<$1>>>)qq_xr2(defn(<<<__SF_ELEM>>>), defn(<<<__SF_REST>>>), <<<$2>>>)>>>)>>>)dnl
define(<<<qq_xr2>>>, <<<qq_xrev(<<<$2>>>, <<<$1>>>ifelse(len(<<<$3>>>), 0, __EE, SP()<<<$3>>>))>>>)dnl
define(<<<qq_xbuild>>>, <<<ifelse(len(<<<$1>>>), 0, <<<$2>>>, <<<qq_split(<<<$1>>>)qq_xb2(defn(<<<__SF_ELEM>>>), defn(<<<__SF_REST>>>), <<<$2>>>)>>>)>>>)dnl
define(<<<qq_xb2>>>, <<<define(<<<__XBA>>>, qq_xloop(<<<$3>>>, <<<$1>>>))qq_xbuild(<<<$2>>>, defn(<<<__XBA>>>))>>>)dnl
dnl qq_xloop(acc, elt): (splice-unquote x) -> (concat x acc); else (cons (qq_expand elt) acc)
define(<<<qq_xloop>>>, <<<ifelse(first_char(<<<$2>>>), LP, <<<split_first(inner_of(<<<$2>>>))define(<<<__XLE>>>, defn(<<<__SF_ELEM>>>))define(<<<__XLR>>>, defn(<<<__SF_REST>>>))ifelse(defn(<<<__XLE>>>), <<<splice-unquote>>>, <<<LP()<<<concat>>>SP()defn(<<<__XLR>>>)SP()<<<$1>>>RP()>>>, <<<define(<<<__QXV>>>, qq_expand(<<<$2>>>))LP()<<<cons>>>SP()defn(<<<__QXV>>>)SP()<<<$1>>>RP()>>>)>>>, <<<define(<<<__QXV>>>, qq_expand(<<<$2>>>))LP()<<<cons>>>SP()defn(<<<__QXV>>>)SP()<<<$1>>>RP()>>>)>>>)dnl
dnl ---- cons / concat / vec ----
define(<<<ev_cons>>>, <<<split_first(skip_ws(<<<$1>>>))ev_cons2(defn(<<<__SF_ELEM>>>), defn(<<<__SF_REST>>>), <<<$2>>>)>>>)dnl
define(<<<ev_cons2>>>, <<<define(<<<__CA>>>, ev_form(<<<$1>>>, <<<$3>>>))ev_cons3(defn(<<<__CA>>>), skip_ws(<<<$2>>>), <<<$3>>>)>>>)dnl
define(<<<ev_cons3>>>, <<<define(<<<__CB>>>, ev_form(<<<$2>>>, <<<$3>>>))ev_cons4(<<<$1>>>, defn(<<<__CB>>>))>>>)dnl
define(<<<ev_cons4>>>, <<<ifelse(len(<<<$2>>>), 2, <<<LP()<<<$1>>>RP()>>>, <<<ifelse(len(inner_of(<<<$2>>>)), 0, <<<LP()<<<$1>>>RP()>>>, <<<define(<<<__C4V>>>, inner_of(<<<$2>>>))LP()<<<$1>>>SP()defn(<<<__C4V>>>)RP()>>>)>>>)>>>)dnl
...
define(<<<concat_join>>>, <<<define(<<<__V>>>, ev_form(<<<$1>>>, <<<$4>>>))concat_scan(skip_ws(<<<$2>>>), ifelse(len(<<<$3>>>), 0, <<<inner_of(defn(<<<__V>>>))>>>, <<<ifelse(len(inner_of(defn(<<<__V>>>))), 0, <<<$3>>>, <<<$3>>>SP()inner_of(defn(<<<__V>>>)))>>>), <<<$4>>>)>>>)dnl
...
define(<<<eq_map>>>, <<<ifelse(first_char(ENC(<<<$2>>>)), <<<{>>>, <<<eq_mp(inner_of(<<<$1>>>), inner_of(<<<$2>>>))>>>, false)>>>)dnl
...
define(<<<ev_read_string2>>>, <<<ifelse(__ERR, 1, __EE, <<<define(<<<__RS>>>, unesc(inner_of(ev_form(defn(<<<__SF_ELEM>>>), <<<$3>>>))))patsubst(patsubst(patsubst(ENC(read_str(translit(defn(<<<__RS>>>), NLCH()TABCH()CRCH(), S1()S2()S3()))), S1(), <<<\\n>>>), S2(), <<<\\t>>>), S3(), <<<\\r>>>)>>>)>>>)dnl
dnl 转义解码：\n → 换行，\t → tab，\r → CR，\" → "，\\ → \，\其它 → 其它
...
define(<<<ev_slurp2>>>, <<<define(<<<__SLF>>>, inner_of(DEC(ev_form(defn(<<<__SF_ELEM>>>), <<<$3>>>))))define(<<<__SL>>>, esyscmd(<<<cat -- '>>>__SLF<<<' | tr '(),\043' '\016\017\020\021'>>>))define(<<<__SLB>>>, __SL)define(<<<__SLS>>>, <<<">>>translit(__SLB, NLCH()TABCH()CRCH(), S1()S2()S3())<<<">>>)patsubst(patsubst(patsubst(defn(<<<__SLS>>>), S1(), <<<\\n>>>), S2(), <<<\\t>>>), S3(), <<<\\r>>>)>>>)dnl
dnl ---- load-file ----
define(<<<ev_load_file>>>, <<<split_first(<<<$1>>>)ev_load_file2(defn(<<<__SF_ELEM>>>), defn(<<<__SF_REST>>>), <<<$2>>>)>>>)dnl
define(<<<ev_load_file2>>>, <<<define(<<<__LFF>>>, inner_of(DEC(ev_form(defn(<<<__SF_ELEM>>>), <<<$3>>>))))define(<<<__LF>>>, esyscmd(<<<cat -- '>>>__LFF<<<' | tr '(),\043' '\016\017\020\021'>>>))define(<<<__LFB>>>, substr(__LF, 0, eval(len(__LF)-1)))define(<<<__LFC>>>, patsubst(__LFB, <<<;.*>>>, <<<>>>))ev_lf_loop(translit(ENC(__LFC), NLCH(), SP()), <<<$3>>>)>>>)dnl
...
define(<<<ev_map2>>>, <<<define(<<<__MF>>>, ev_form(<<<$1>>>, <<<$3>>>))ev_map3(defn(<<<__MF>>>), skip_ws(<<<$2>>>), <<<$3>>>)>>>)
define(<<<ev_map3>>>, <<<define(<<<__ML>>>, ev_form(<<<$2>>>, <<<$3>>>))map_scan(inner_of(defn(<<<__ML>>>)), <<<$1>>>, __EE, <<<$3>>>)>>>)dnl
...
define(<<<sf_group_close>>>, <<<ifelse(<<<$1>>>, 1, <<<sf_done(<<<$3>>>first_char(<<<$2>>>), skip_ws(rest_str(<<<$2>>>)))>>>, <<<sf_group(eval(<<<$1>>>-1), rest_str(<<<$2>>>), <<<$3>>>first_char(<<<$2>>>))>>>)>>>)dnl
dnl ---- string literal ----
