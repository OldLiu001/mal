...
> 本实现的一切取舍以**运行效率优先**为第一原则：剥离无用抽象，保证简洁有效。

1. **变量数是第一成本**：cmd 环境表每多一个变量，`set`/枚举/`call`/跨进程全部变慢（实测见 §3.5）。**能用代码行换的变量就用代码行换；能不引入的变量坚决不引入。**
2. **函数模板/宏抽象不免费**：把短逻辑包成「调用/宏/模板」省了重复代码，却引入跨文件子进程、间接寻址与变量传递开销。**小对象/短逻辑应内联**；只在确实高频复用且不损失速度时才保留函数模板（见 §8 ROI 清单）。
3. **热路径扁平**：热循环用「括号块 + 延迟展开」禁 goto；高频子例程标签靠前；文件总行数受控。
4. **环境规模动态受限**：对象存储需一个**运行时探测的 cmd 变量容量上限的可调全局阈值**兜底，防止环境膨胀击穿性能与稳定性（容量因机器而异，必须可动态调整）。
...
**不可行边界（2026-08-24）**：`%<-%`/`%->%` 能否进一步 merge 成"无参宏内联"（调用点直接展开逻辑，连 `call :`
都省掉）？**不能**。（1）SetRet 语义含 NSMeta 跨层句柄记账：需读调用栈帧并 `RefCnt++`，正确性依赖当前递归
层级 `_G.LEVEL`/`_P.<LEVEL>.*` 上下文，纯宏文本在任意调用点展开无法稳定携带该上下文。（2）GetRet 要把结果
写回调用方指定的返回值槽（`_P.<LEVEL>.OUT`），宿主函数临时变量命名空间 `_T.<FN>.*` 各不相同，内联即与调用点
的顶层变量名冲突/串扰。（3）两宏被全部模块每个函数逐调用使用，若内联则同段逻辑复制到数百处 → 编码爆炸 +
`.for-var` 共享域串扰复发（§8.4.3.4），与 #1/#9 的"单点收敛"背道而驰。故**"每模块内置一个同进程单例 label +
宏改 util.bat 一处"即内联投资边界**；再往深处去 call 需整体换架构（如 §8.2.1 PACKED 真扁平），ROI 为负暂缓。
