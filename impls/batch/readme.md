...
## 9.2 打包入口转发层设计（调用点方案，readline/writeall 不动）
...
- **方向澄清（本会话定稿）**：曾验证"文件头 `%*` 分包层 + 同名函数"的**调用点转发范式的
  机制可行性**（见下 9.5），但这**不意味着**要给 readline/writeall 本体做真函数化——它们是
  顺序执行黑盒，按 9.0~9.2 只需靠打包入口转发头命中，**本体保持原样**更好（不触碰易碎转义
  逻辑）。9.5 的样板纯粹用于验证"`%*` 分包 + 标签转发"这条路能成立。

## 9.5 令牌转发样板验证（writeall，本会话实测）
