...
:MAIN_Eval ObjMal Env [Tail] -> ObjMal
	for %%. in (_L[!_G.LEVEL!].) do (
		set "%%.ObjMal=%~1"
		set "%%.Env=%~2"
		set "%%.Tail=%~3"
		...
		if "!%%.FnType!" == "MalFn" (
			...
			if "!%%.SubType!" == "MAL" (
				...
				if "!%%.Tail!" == "1" (
					...
					set "_G.LEVEL[!_G.LEVEL!][!%%.Args!]="
					for /l %%j in (2 1 !%%.AI!) do (
						%{g% "!%%.Args!" Item[%%j] %%.TCOItem %}%
						set "_G.LEVEL[!_G.LEVEL!][!%%.TCOItem!]="
					)
					set "%%.RetMal=!%%.Fn!"
				) else (
				...
				%{g% "!%%.Fn!" Mod %%.Mod %}%
				%{g% "!%%.Fn!" Name %%.Name %}%
				call :!%%.Mod!_!%%.Name! "!%%.Args!" "!%%.Env!" "!%%.Tail!" %->% %%.RetMal
				%?% (
					%-|%
				)
			)
...
:MAIN_ApplyClosure FnMal ObjMal -> RetMal
	...
	set "_G.TCO.Fn="
	set "_G.TCO.Args="
	:MAIN_ApplyClosure_TCO_Loop
	for %%. in (_L[!_G.LEVEL!].) do (
		if not defined %%.TCOInit (
			set "%%.TCOInit=1"
			set "%%.Fn=%~1"
			set "%%.Obj=%~2"
		)
...
:MAIN_MIf Mal Env [Tail] -> Mal
	for %%. in (_L[!_G.LEVEL!].) do (
		...
		if "!%%.TF!" == "1" (
			%{g% "!%%.Mal!" Item[3] %%.Then %}%
			%{% MAIN Eval "!%%.Then!" "!%%.Env!" "!%%.IfTail!" %}% %->% %%.RetMal
			%?% (
				%-|%
			)
		) else (
			if !%%.Count! gtr 3 (
				%{g% "!%%.Mal!" Item[4] %%.Else %}%
				%{% MAIN Eval "!%%.Else!" "!%%.Env!" "!%%.IfTail!" %}% %->% %%.RetMal
				%?% (
					%-|%
				)
			) else (
...
:MAIN_MLet Mal Env [Tail] -> Mal
	for %%. in (_L[!_G.LEVEL!].) do (
		...
		%{g% "!%%.Mal!" Item[3] %%.Body %}%
		%{% MAIN Eval "!%%.Body!" "!%%.NewEnv!" "!%%.LetTail!" %}% %->% %%.RetMal
		%?% (
			%-|%
		)
		%<-% %%.RetMal
