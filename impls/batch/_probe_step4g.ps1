...
$cases = @(
  @{ name="def+use"; lines=@("(def! a 5)","a") },
  @{ name="def-fn+call"; lines=@("(def! f (fn* (a) a))","(f 5)") },
  @{ name="do-def"; lines=@("(do (def! a 6) 7 (+ a 8))") },
  @{ name="sumdown"; lines=@("(def! sumdown (fn* (N) (if (> N 0) (+ N (sumdown (- N 1))) 0)))","(sumdown 1)","(sumdown 2)","(sumdown 6)") },
  @{ name="fib"; lines=@("(def! fib (fn* (N) (if (= N 0) 1 (if (= N 1) 1 (+ (fib (- N 1)) (fib (- N 2)))))))","(fib 1)","(fib 2)","(fib 4)") },
  @{ name="let-fn-closure"; lines=@("(let* (f (fn* () x) x 3) (f))") },
  @{ name="let-cst"; lines=@("(let* (cst (fn* (n) (if (= n 0) nil (cst (- n 1))))) (cst 1))") },
  @{ name="let-mutual"; lines=@("(let* (f (fn* (n) (if (= n 0) 0 (g (- n 1)))) g (fn* (n) (f n))) (f 2))") },
